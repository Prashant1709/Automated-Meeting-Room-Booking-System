      ...
      rooms.forEach((room) => {
        ...
        roomCard.innerHTML = `
          ...
          <div class="room-details">
              ...
              <p class="amenities-list">Amenities: ${Object.keys(room.amenities).join(", ")}</p>
              <p class="room-info">Total Cost per Hour: $${Object.values(room.amenities).reduce((a, b) => a + b, 0)}</p>
              ${room.status === "booked" ? `<div class="timer" id="timer-${room.meetingRoomId}"></div>` : ""}
              ${!room.status === "available" ? `<a href="bookRoom.html?roomId=${room.meetingRoomId}" class="book-room-btn">Book Room</a>` : ""}
          </div>
        `;
